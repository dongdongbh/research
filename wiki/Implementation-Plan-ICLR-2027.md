...
- **RoboArena dump is small: 18.5 GB** (`RoboArena/DataDump_02-03-2026`,
  `usedStorage` = 18,473,184,727 B; MIT; 23.6K metadata rows in the parquet
  export). Fallback `DataDump_08-05-2025` = 8.1 GB. The prereg's "dump size
...
- **Cosmos-Reason available**: `nvidia/Cosmos-Reason1-7B` (Qwen2.5-VL base,
  plain transformers) and newer `Cosmos-Reason2-2B/8B` (Dec 2025, Qwen3-VL
...
2. Models: `teetone/RoboReward-4B`, `teetone/RoboReward-8B`,
   ...
   `nvidia/Cosmos-Reason2-8B` (license note), SigLIP2 (shared below).
3. DreamGen Bench assets: the four published video models, bench prompts,
...
2. `wusize/clipself` checkpoint (HF or GitHub release) — the A4 week-1
   decisive check.
3. A5 concretizations from the wave-2 gate: LABCLIP (arXiv 2502.03566) —
   ~590K-param text-side matrix, reimplement in a day if unreleased; DCSM
   (ICCV 2025) — check code release; DeGLA checkpoint — check release,
