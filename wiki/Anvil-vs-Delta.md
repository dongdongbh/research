# Anvil vs. Delta, DeltaAI, and Bridges-2

Updated 2026-08-02 for the general research wiki (renamed from
`Anvil-vs-Delta-for-SVIB2`).

Purpose: which cluster a given workload should run on, with first-party
...
Keep **Anvil as the lab's primary system**. Repositories, Python environments,
generated corpora, feature artifacts, and benchmark metadata are already there;
COCO and Visual Genome are available in Anvil's shared AI dataset collection;
and AnvilGPT provides a hosted LLM endpoint directly.
...
| Workload | First choice | Why | When to move elsewhere |
|---|---|---|---|
| Hosted-LLM (teacher/judge) API generation | **AnvilGPT from an allocated CPU compute job**, or a non-HPC client | No local GPU is required. Anvil explicitly says Slurm must be used for work and production work on login nodes violates policy. [Anvil job policy](https://docs.rcac.purdue.edu/userguides/anvil/jobs/#overview-slurm-basics) | Move the API client only if another legitimate CPU execution host is easier. Moving to another GPU system does not speed a remote rate-limited API. |
| Self-hosted ~30B-class model on vLLM | **One Anvil H100 first** | Lowest migration cost; Anvil has 21 four-H100, 80-GB nodes and permits shared GPU nodes. [Anvil architecture](https://docs.rcac.purdue.edu/userguides/anvil/architecture/#compute-nodes), [accounting](https://docs.rcac.purdue.edu/userguides/anvil/jobs/#gpu-nodes) | Try one Delta H200 if 80 GB is genuinely too tight, or DeltaAI after an ARM container smoke. Do not request multiple nodes for a single 27B-class model. |
...
4. **Bridges-2**: credible x86 H100 overflow capacity, but no advantage large
   enough to repay a second data/software migration today.

The svib2 project applied exactly this ranking in July 2026: Anvil primary,
Delta bootstrapped as a second site, DeltaAI and Bridges-2 deferred. Full
detail: svib2 repo wiki, page Anvil-vs-Delta-for-SVIB2.
...
### How many GPUs for a 27B-class dense model?
...
| System | Production/debug access | Charging facts that matter |
|---|---|---|
...
For a long hosted-API corpus run, the clean solution is a small Anvil CPU
...
| System | Persistent/work storage | Consequence |
|---|---|---|
| **Anvil** | Home 25 GB with snapshots, scratch 100 TB with no backup and a 30-day purge, project 5 TB with daily snapshots. [RCAC storage table](https://docs.rcac.purdue.edu/userguides/anvil/architecture/#storage) | Best current locality: repos and artifacts are already in project space and COCO/VG can remain symlinks into the shared collection. Use scratch for transient extraction, not irreplaceable H5 caches. |
| **Delta/DeltaAI** | Home 100 GB; projects start at 500 GB and can be requested up to 25 TB; work starts at 1 TB and can be requested up to 100 TB; **our `/work/hdd/bhvn` is 8 TB as of 2026-09-15**, so datasets download there (NVMe, 3 TB since 2026-09-16, holds environments and weights; copy-then-remove between tiers, never `mv`). DeltaAI says HDD and NVMe work areas are shared between Delta and DeltaAI. [Delta storage](https://docs.ncsa.illinois.edu/systems/delta/en/latest/user_guide/data_mgmt.html#file-systems), [DeltaAI storage](https://docs.ncsa.illinois.edu/systems/deltaai/en/latest/user-guide/data-mgmt.html#file-systems) | Attractive if both NCSA systems are used because one staged corpus can serve both. Projects/work have no snapshots, so keep durable copies elsewhere. |
| **Bridges-2** | Each allocation includes persistent Ocean storage; node-local `$LOCAL` is deleted after the job. [PSC file-space guide](https://www.psc.edu/resources/bridges-2/user-guide/#file-spaces) | Fine operationally, but image data, model cache, repo, and features must all be transferred and rebuilt or relinked. |
...
- **Anvil and Delta are x86-64**, so a project's `uv` lock and CUDA wheels have
...
- **DeltaAI is `aarch64`**. NCSA explicitly warns that many Python packages may
  ...
  reason DeltaAI is not the easiest first move.
...
AnvilGPT is the exception relevant to Anvil work: RCAC directly documents its
...
from batch inference and may require a site's persistent-service facility; none
is needed for offline batch corpus generation.

## Standing checklist before moving a workload

1. Benchmark the **single-Anvil-H100** configuration first and record startup
   memory, peak memory, throughput, and SU per unit of accepted output (worked
   example: [[Anvil-H100-Qwen36-vLLM-Benchmark]]).
2. Compare it against the hosted API rate on the same fixed input sample.
3. If self-hosting wins, run one H100 with multiple request streams; add GPUs
   only after a 1-vs-2-vs-4 GPU throughput benchmark shows lower SU per unit of
   output.
4. Request/exchange a small Anvil **CPU** allocation for remote-API generation
   so no production workload runs on login nodes and no idle H100 is reserved.
5. Revisit DeltaAI only when feature extraction or training demonstrates useful
   multi-GPU scaling; first require a complete test-suite run and a vLLM
   container smoke on ARM.

svib2's July 2026 resolution of this checklist (local-vs-hosted generation
verdict and the authorized generation lanes) lives in the svib2 repo wiki,
pages Anvil-vs-Delta-for-SVIB2 and Current-Standard.
