...
|---|---|---|
| ICLR (Sep 18) | **RoboJudge audit** ★★★★½ — do robot-policy evaluators recover the human ranking | **RESULTS FINAL (Aug 10): H1 true — 4 of 6 evaluators name the wrong best policy; SigLIP2 significantly ANTI-correlated (τ −0.714); H5 in strongest form; contamination (+0.10 τ) caught by double-reporting; per-judge axis-named noise floors. Campaign closed and archived; WRITING PHASE (abstract Sep 18).** See [[Prereg-RoboJudge-Audit]] |
...
| CVPR (~Nov 13) | **1-NFE diversity** | **RESULTS FINAL (Aug 10): H1 confirmed in Shortcut (13/13, +0.17 at 50k) but NOT in iMF — model-dependent, so not an objective property; H2 true (+0.215, CI excludes zero) but its mechanism unsupported; central finding: training recipe dominates, objective family predicts nothing. 40 cells, paired intervals, 4 self-corrected errors ledgered. H3 launch spec delivered (knob confound pre-declared; needs its own compute window). WRITING PHASE.** See [[Prereg-1NFE-Diversity]] |
...
| ICML (~Jan 28) | **Epistemic contextualization** | **STOPPED BY ITS OWN PRE-REGISTERED GATE (Aug 10):** the treatment rewriter preserves facts on 91.2% vs a 97% threshold frozen before judging, while the same model's placebo paraphrase scores 97.1% — the attribution mechanism itself INVENTS sources when metadata is thin (a mechanism-level finding; §5 anticipated a publishable negative). All infrastructure banked and resumable; LR locked; DELTA_LAUNCH.md ready behind a blocking notice. **Owner decides: honest-note paper vs amended method (stronger rewriter / retrieval-grounded sources) — plus the 50-item blinded human spot-check the prereg requires.** See [[Prereg-Epistemic-Contextualization]] |
...
| **NEW candidates (ungated): weight-space learning, 5 rows** | pre-gate, stars run hot | 3 zero-GPU reading checks first | See [[Weight-Space-Learning-Scout-2026-08]] — owner-requested scout (Aug 9). Best-graded: D4, the sparsity-premise zoo dual-use (our 1,800 planned checkpoints carry an EXACTLY computed safety label — a weight-space dataset nobody else can make, ~2–5 GPU-h marginal, the only candidate not in a race); D1/D3 (robustness of weight-only safety screening) are real but narrowed by [Z-PEFT](https://arxiv.org/abs/2608.02271), posted six days before the scout. Generation at LLM scale is closed; representation-at-scale is a race against named groups. Eleven do-not-do cells listed. Three reading checks (Z-PEFT full text, Borth's OOD gap, sparsity checkpoint format) decide whether any gate runs |
